...
扩展集成测试可以在CI服务上运行。
The [`@vscode/test-electron`](https://github.com/microsoft/vscode-test) 库可帮助您在CI提供程序上设置扩展测试，并包含Azure pipeline上的[示例扩展](https://github.com/microsoft/vscode-test/tree/main/sample)设置。
你可以看看 [build pipeline](https://dev.azure.com/vscode/vscode-test/_build?definitionId=15) 或者直接跳转到[`azure-pipelines.yml` 文件](https://github.com/microsoft/vscode-test/blob/main/sample/azure-pipelines.yml).

## 自动发布

您还可以配置CI以自动发布新版本的扩展。

publish命令类似于从本地环境使用 [`vsce`](https://github.com/microsoft/vscode-vsce), 但您必须以某种安全的方式提供个人访问令牌(PAT)。
通过将PAT存储为`VSCE_PAT`秘密变量，`vsce`将能够使用它。
秘密变量永远不会公开，因此在CI管道中使用它们是安全的。
