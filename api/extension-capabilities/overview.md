...
用户或扩展提供的自定义样式表将针对DOM结构和类名工作。因为我们认为它们是内部的，所以没有文档化。
为了进化、重构或改进VS Code，我们需要对用户界面进行修改的自由。
对DOM的任何更改都可能破坏现有的自定义样式表，导致样式表提供者感到沮丧，并且由于损坏的样式表而导致UI故障，从而导致糟糕的用户体验。

相反，VS Code旨在提供一个设计良好的扩展API，支持UI自定义。
API是文档化的，附带工具和示例，并且在所有即将发布的VS Code版本中保持稳定。
